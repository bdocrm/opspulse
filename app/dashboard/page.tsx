...
import { CountUp, ViewportRevealGroup } from "@/components/motion/dashboard-motion";
...
function KpiCard({ title, value, valueInfo, target, variance, trend, status, tone, progress, icon: Icon, loading }: {
  ...
  valueInfo?: { number: number; format: (n: number) => string };
}) {
  return (
    <Card data-reveal-item className="motion-stagger-item motion-hover-lift group overflow-hidden hover:border-primary/30">
      <CardContent className="p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">{title}</p>
            {loading ? <Skeleton className="mt-3 h-9 w-32" /> : valueInfo ? <p className="mt-2 truncate text-3xl font-bold tracking-tight" title={valueInfo.format(valueInfo.number)}><CountUp value={valueInfo.number} format={valueInfo.format} /></p> : <p className="mt-2 truncate text-3xl font-bold tracking-tight" title={value}>{value}</p>}
          </div>
...
export default function DashboardPage() {
  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-6">
      <header className="grid gap-4 xl:grid-cols-[minmax(260px,1fr)_minmax(620px,auto)] xl:items-start">
        ...
        <div className="rounded-2xl border border-border/70 bg-card p-3">
          <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-[minmax(190px,1fr)_140px_100px_auto_auto]">
            <CampaignSelector campaigns={campaigns} selectedCampaignId={selectedCampaignId} onCampaignChange={setSelectedCampaignId} includeAllOption labelClassName="sr-only" triggerClassName="h-10" className="min-w-0 space-y-0" />
      ...
      <section aria-labelledby="kpi-heading"><h2 id="kpi-heading" className="sr-only">Key performance indicators</h2><ViewportRevealGroup initialDelay={90} className="motion-stagger grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard title="Total MTD" value={rows.length === 0 ? "No production data" : formatCurrency(data?.kpis.totalMTD)} valueInfo={data?.kpis.totalMTD != null ? { number: data.kpis.totalMTD, format: formatCurrency } : undefined} target={totalGoal == null ? "Target not configured" : formatCurrency(totalGoal)} variance={data?.kpis.totalMTD != null && totalGoal != null ? formatCurrency(data.kpis.totalMTD - totalGoal) : "Not available"} trend={data?.kpis.totalMTD != null && previousKpis?.totalMTD != null && previousKpis.totalMTD !== 0 ? ((data.kpis.totalMTD - previousKpis.totalMTD) / Math.abs(previousKpis.totalMTD)) * 100 : null} status={overall.status} tone={overall.tone} progress={data?.kpis.avgAchievement} icon={Target} loading={isLoading} />
        <KpiCard title="Achievement" value={data?.kpis.avgAchievement == null ? "Target not configured" : formatPct(data.kpis.avgAchievement)} valueInfo={data?.kpis.avgAchievement != null ? { number: data.kpis.avgAchievement, format: formatPct } : undefined} target="100.0%" variance={data?.kpis.avgAchievement == null ? "Not available" : signedPoints(data.kpis.avgAchievement - 100)} trend={data?.kpis.avgAchievement != null && previousKpis?.avgAchievement != null ? data.kpis.avgAchievement - previousKpis.avgAchievement : null} status={overall.status} tone={overall.tone} progress={data?.kpis.avgAchievement} icon={TrendingUp} loading={isLoading} />
        <KpiCard title="Run Rate" value={data?.kpis.avgRunRate == null ? "No production data" : formatCurrency(data.kpis.avgRunRate)} valueInfo={data?.kpis.avgRunRate != null ? { number: data.kpis.avgRunRate, format: formatCurrency } : undefined} target={totalGoal == null ? "Target not configured" : formatCurrency(totalGoal)} variance={data?.kpis.avgRunRate != null && totalGoal != null ? formatCurrency(data.kpis.avgRunRate - totalGoal) : "Not available"} trend={data?.kpis.avgRunRate != null && previousKpis?.avgRunRate != null && previousKpis.avgRunRate !== 0 ? ((data.kpis.avgRunRate - previousKpis.avgRunRate) / Math.abs(previousKpis.avgRunRate)) * 100 : null} status={getExecutiveStatus({ hasData: measurable.length > 0, goal: totalGoal, achievement: data?.kpis.avgRRAchievement ?? null }).status} tone={getExecutiveStatus({ hasData: measurable.length > 0, goal: totalGoal, achievement: data?.kpis.avgRRAchievement ?? null }).tone} progress={data?.kpis.avgRRAchievement} icon={Activity} loading={isLoading} />
        <KpiCard title="Run Rate Achievement" value={data?.kpis.avgRRAchievement == null ? "Target not configured" : formatPct(data.kpis.avgRRAchievement)} valueInfo={data?.kpis.avgRRAchievement != null ? { number: data.kpis.avgRRAchievement, format: formatPct } : undefined} target="100.0%" variance={data?.kpis.avgRRAchievement == null ? "Not available" : signedPoints(data.kpis.avgRRAchievement - 100)} trend={data?.kpis.avgRRAchievement != null && previousKpis?.avgRRAchievement != null ? data.kpis.avgRRAchievement - previousKpis.avgRRAchievement : null} status={getExecutiveStatus({ hasData: measurable.length > 0, goal: totalGoal, achievement: data?.kpis.avgRRAchievement ?? null }).status} tone={getExecutiveStatus({ hasData: measurable.length > 0, goal: totalGoal, achievement: data?.kpis.avgRRAchievement ?? null }).tone} progress={data?.kpis.avgRRAchievement} icon={BarChart3} loading={isLoading} />
      </ViewportRevealGroup></section>
